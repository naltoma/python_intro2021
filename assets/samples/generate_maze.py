...
def create_maze(maze, x, y, random_seed=0):
    ...
    direction=3, diff_x=-2, diff_y=0 (direction: 0=North, 1=East, 2=South, 3=West)
    direction=3, diff_x=-2, diff_y=0 (direction: 0=North, 1=East, 2=South, 3=West)
    >>> print_maze(maze)
    ----
    1100
    ...
    while(True):
        direction, diff_x, diff_y = set_direction()
        print(f'{direction=}, {diff_x=}, {diff_y=} (direction: 0=North, 1=East, 2=South, 3=West)')
        flag = is_space(maze, x, y, diff_x, diff_y)
        if flag == True:
            x, y = dig_maze(maze, x, y, diff_x, diff_y)
            #print('new(x,y) = ', x, y) # view step by step
            #print_maze(maze)
        else:
            break
