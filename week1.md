...
![プログラミングに含まれる3ステップ](./figs/w1-programming.svg)
...
- コード例を動かす手順
  - step 1: 「ターミナル」を起動。
    ...
    - Dockへ登録しておくと便利。
  - step 2: Python仮想環境に入る。
    - ``source ~/.venv/prog1/bin/activate`` と入力。
    - 上記コマンドを実行するとターミナルのプロンプトが ``(prog1)`` のように変化するはずだ。変化しなかった場合、コマンドを間違えている可能性が高い。
      - 2020年度以前の再履修生はこの手順は不要です。Anaconda/Minicondaで環境構築しているため、スキップしてください。
  - step 3: Pythonインタプリタの起動。
    - ``python`` と入力。
  - step 4: インタプリタにコードを入力し、Enterキー（=実行）。
    - e.g., 最初のコード例, 2.1節
      - ``print(‘Yankees rule!’)``
  ...
  - 上記 step 1〜5 を数回繰り返そう。これで教科書等のコード例をすべて動かすことができる。
- 2.1節コード例の補足
...
![演習2：オブジェクトと式と型](./figs/w1-ex2.svg)
...
```{note}
どうおかしいのかに関する説明が後ろに続いている。「unsupported operand type(s) for +: 'int' and 'str'」とは「int（数字のこと）とstr（文字列のこと）を+演算子で操作することはサポート外だ」と伝えている。このようなコンピュータからの応答を少しずつ読めるようになろう。応答分の意図を汲み取ることができるようになると、どこがどうおかしいのか検討がつきやすくなり、プログラムの修正に役立てやすくなる。このやり取りがコンピュータとの対話を通した開発という意味である。
```

````{note}
型を把握することはとても大切であり、ここでは **基本的な型として int, float, str** をひとまず覚えておき、必要に応じて教科書を参照して別の型についても調べられるようにしておこう。また、型の調べ方として ``type()`` を覚えておこう。type関数は後述する変数に対しても適用できる。
````

```{tip}
Python標準で用意されている型は[公式ドキュメントの組み込み型](https://docs.python.org/ja/3.8/library/stdtypes.html)を参照してみよう。現時点ではint, str, boolぐらいを扱い、今後少しずつ他の型を学んでいきます。
```

---
## 演習3：変数と等号の利用
![演習3：変数と等号の利用](./figs/w1-ex3.svg)
...
```{note}
変数名は **英数字やunderscore（``_``のこと）を組み合わせて使う** ことが一般的だが、冒頭に数字を用いることはできない点に注意すること。実際に試してみよう。
```
...
- 事前準備
  - [テキストエディタAtomのインストール](./week2.html#atom)
