...
上記はループ文が入れ子構造になっているコード例を示している。どのように出力されるか想像できるだろうか。[デバッグ実行](http://ie.u-ryukyu.ac.jp/~tnal/2021/prog1/vscode.pdf)で1行目にブレイクポイントを設定し、step in実行すると実行行がどのように移動するのか、その最中の変数がどのように変わっていくのかを確認しやすい。
